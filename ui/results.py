...
from utils.overrides import get_overrides, save_override, delete_overrides, generate_item_key
...
def render_results(
    ...
    if snapshot_id and overrides:
        if st.button("🔄 Resetar Edições Manuais", help="Remove todos os ajustes manuais e volta para a sugestão original da IA"):
    ...
    with st.expander("🧠 Contexto e Valor Entregue", expanded=True):
        ...
        m4.metric("Etapa AARRR", context.get("etapa_funil", "N/A"), help="Posicionamento no funil de produto")
        
        st.markdown(f'<div class="mf-premium-metric" style="margin-top:1.5rem; margin-bottom:1.5rem;"><strong>📝 Resumo do PRD:</strong><br>{context.get("resumo_prd", "N/A")}</div>', unsafe_allow_html=True)
    ...
    with st.expander("📈 L1 Health Indicators (Saúde do Produto)", expanded=True):
        for idx, l1 in enumerate(metrics.get("l1_health_indicators", [])):
            ...
            if st.session_state.get("editing_item") == editing_key:
                new_val = c2.text_input("Nome da Métrica", value=display_metrica, key=f"input_{editing_key}")
        ...
        for okr_idx, okr in enumerate(metrics.get("okrs", [])):
            ...
            for kr_idx, kr in enumerate(okr.get('key_results', [])):
                ...
                with col_kr:
                    ...
                    if st.session_state.get("editing_item") == editing_key:
                        new_meta = st.text_input("Ajustar Meta", value=display_meta, key=f"input_{editing_key}")
            ...
            if st.session_state.get("editing_item") == add_kr_key:
                ...
                if col_s.button("Salvar KR", key=f"save_{add_kr_key}"):
                    if new_res.strip():
                        ...
                        added_krs.append({"resultado": new_res, "meta": new_meta, "baseline": "N/A"})
            ...
            for akr_idx, akr in enumerate(added_krs):
                ...
                with col_adel:
                    if st.button("🗑️", key=f"del_added_{okr_key}_{akr_idx}", help="Remover KR manual"):
    ...
    st.markdown("---")
    st.markdown('<div style="padding: 1.5rem; background: rgba(99,102,241,0.05); border-radius: 12px; border: 1px solid rgba(99,102,241,0.1);">', unsafe_allow_html=True)
