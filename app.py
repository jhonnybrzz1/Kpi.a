...
with st.sidebar:
    ...
    selected_category = st.selectbox(
        "🏷️ **Escolha uma Categoria:**", ["Todas as Categorias"] + categories, index=0
    )
    ...
    if selected_category == "Todas as Categorias":
        ...
        filtered_examples = {
            k: v for k, v in INITIATIVE_EXAMPLES.items() if v.get("category") == selected_category
        }
...
def main():
    ...
    with col_stat1:
        st.metric(label="🤖 Modelos IA", value="2", delta="Mistral + GPT-4")

    with col_stat2:
        st.metric(label="📊 Tipos de Métricas", value="50+", delta="KPIs Personalizados")

    with col_stat3:
        st.metric(label="⚡ Tempo Médio", value="45s", delta="Análise Completa")

    with col_stat4:
        st.metric(label="📄 Formato Saída", value="PDF", delta="Profissional")
        ...
        with processing_container:
            ...
            try:
                ...
                with progress_container:
                    ...
                    metrics_analysis = openai_service.generate_metrics(
                        clean_input, context_analysis
                    )
                    ...
                    if metrics_analysis:
                        ...
                        if metrics_analysis.get("kpis"):
                            ...
                            for i, kpi in enumerate(metrics_analysis["kpis"], 1):
                                st.write(
                                    f"{i}. **{kpi.get('nome', 'KPI')}:** {kpi.get('descricao', 'N/A')}"
                                )
                                if kpi.get("formula"):
                                    st.code(kpi["formula"])
